...
crossF v@(Vector3 x y z) v'@(Vector3 x' y' z') = isZero ? Nothing $ Just (Vector3 x0 y0 z0) 
  where
    ...
    y0 = (head . tail) ls
    z0 = last ls
    isZero = (abs x0) < epsilon && (abs y0) < epsilon && (abs z0) < epsilon 
...
drawSphereNX::Int -> GLfloat ->Int -> Bool -> [Color3 GLdouble] -> IO()
drawSphereNX n radius k isFilled cc = do
    let δ = 2*pi / rf n :: GLfloat
        ϵ = pi / rf n :: GLfloat
        r = radius
        fx::Int -> Int -> GLfloat
...
{-|
  ...
  NOTE: take k latitude from the North
    ...
    @
 -}
drawParamSphereX::Fx -> Fy -> Fz -> Int -> Int -> Bool -> [Color3 GLdouble]-> IO ()
drawParamSphereX fx fy fz n k isFilled cc = do
  preservingMatrix $ do
        let mx = combinePt ss cc 
        mapM_ (\(k, v) -> drawSegmentFromTo (odd k ? red $ white) v
              ) $ zip [1..] ss
        when isFilled $ do       
        ...
        -- NOTE: North and South pole pt are singular pts
        let lxx = [[
                    {--
                    [
                      (Color3 0.0 1.0 0.0,Vertex3 (-1.7484556e-8) (-0.0) 0.4),
        ...
        when True $ do       
          ...
          (mapM_ . mapM_) (\tr -> do 
                                  ...
                                  let (a:b:c:_) = tr
                                  let pt = let (a:b:c:_) = fmap snd tr in intersectLineTri (p0, p1) (a, b, c)
                                  case pt of
                                    Just x -> do 
                                      logFileG ["intersection_yes"] 
                                      logFileG [show x] 
                                      -- translate (vec_ x)
                                      drawCubeQuad 0.1
                                    Nothing -> do
                                      logFileG ["intersection_no"]
                                                     
                           )  $ map (\x -> listSlide x 3) mx 
      where
        ss = [[Vertex3 (fx i j)
                       (fy i j)
                       (fz i j) | i <- take 4 [0..n]] | j <- let m = div n 2 in take (k+1) [m, m - 1 .. -m]]
